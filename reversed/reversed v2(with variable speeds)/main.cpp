...
void newwall(pointer& wallsofgame,bool firsttime,int& before)
{
	wall a;
	a.initialise(firsttime,before);
	if(firsttime)
	before=a.returnx();
...
int main()
{
	...
	newwall(wallsofgame,firsttime,before);     //the first wall was added
	firsttime=0;
	wallrand=rand()%4+5;
	int reverserand;
	...
	{
		//while(!_kbhit())
			for(;!_kbhit();i++,vartodeletewall++,j++,k++)
			{

				
				if(i%9==wallrand)     //if the time has come for a new wall to be made
				{
					newwall(wallsofgame,firsttime,before);
					i=0;
					wallrand=rand()%4+5;
				}
